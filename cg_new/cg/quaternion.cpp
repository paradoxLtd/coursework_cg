...
Quaternion::Quaternion(Matrix4f &rot)
{
    ...
    else
    {
        ...
        {
            double s = 2.0f * (double)Math.sqrt(1.0f + rot.Get(0, 0) - rot.Get(1, 1) - rot.Get(2, 2));
...
Quaternion Quaternion::NLerp(Quaternion &dest, double lerpFactor, bool shortest)
{
...
Quaternion Quaternion::SLerp(Quaternion &dest, double lerpFactor, bool shortest)
{
    ...
    double sin = sqrt(1.0f - cos * cos);
    double angle = atan2(sin, cos); //what is this?)
