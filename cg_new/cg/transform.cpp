...
Transform::Transform()
{
    return Transform(Vector4f(0, 0, 0, 0));
}

Transform::Transform(Vector4f &pos)
{
    return Transform(pos, Quaternion(0, 0, 0, 1), Vector4f(1, 1, 1, 1));
}
...
Quaternion Transform::GetLookAtRotation(Vector4f &point, Vector4f &up)
{
    ...
    return Quaternion(mtx.InitRotation(point.Sub(m_pos).Normalized(), up));
}
